...
export function ScorersTab({ tournament }: { tournament: Tournament }) {
  const { t } = useI18n();
                  ...
                  {matchBreakdown.map(({ match, goalsInMatch }) => {
                    ...
                    return (
                      <div key={match.id} style={{
                        ...
                        {match.status === 'live' && (
                          <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--danger)', flexShrink: 0 }}>{t('tournament.detail.liveLabel')}</span>
                        )}
                      </div>
                    );
                  })}
