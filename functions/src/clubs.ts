 ...
 * - migrateUserClubs: one-time migrace legacy /users/{uid}/clubs → /clubs/{clubId}
 *
...
// ─── Migration: legacy /users/{uid}/clubs → /clubs/{clubId} ────────────────

/**
 * Jednorázová migrace legacy per-user klubů do sdíleného modelu.
 * - Všechny kluby z /users/{uid}/clubs/* se přesunou do /clubs/{newClubId}
 * - Uživatel se stane ownerem každého klubu
 * - /trainings/{uid}/* → /trainings/{newClubId}/* (matched by training.clubId nebo first club)
 * - /matches/{uid}/* → /matches/{newClubId}/* (matched by match.clubId)
 * - /contacts/{uid}/* → /contacts/{newClubId}/* (matched by contact.clubId)
 * - /users/{uid}/activeClubId = první migrated club
 * - Po migraci /users/{uid}/clubs, /trainings/{uid}, /matches/{uid}, /contacts/{uid} zůstanou
 *   jako backup (nemažeme automaticky — admin to může udělat ručně po verifikaci)
 *
 * Volatelné:
 * - samotným uživatelem (migruje vlastní data)
 * - adminem (migruje data konkrétního uživatele via targetUid)
 * - dryRun flag vrací preview bez zápisu
 */
export const migrateUserClubs = functions.region('europe-west1').https.onCall(async (data, context) => {
  const callerUid = requireAuth(context);
  const targetUid = sanitizeString(data?.targetUid, 128) || callerUid;
  const dryRun = data?.dryRun === true;

  if (targetUid !== callerUid && callerUid !== ADMIN_UID) {
    throw new functions.https.HttpsError('permission-denied', 'Can only migrate own data (or admin)');
  }

  // Načti legacy data
  const [legacyClubsSnap, legacyTrainingsSnap, legacyMatchesSnap, legacyContactsSnap] = await Promise.all([
    db.ref(`users/${targetUid}/clubs`).get(),
    db.ref(`trainings/${targetUid}`).get(),
    db.ref(`matches/${targetUid}`).get(),
    db.ref(`contacts/${targetUid}`).get(),
  ]);

  const legacyClubs = (legacyClubsSnap.val() || {}) as Record<string, SharedClub>;
  const legacyClubIds = Object.keys(legacyClubs);

  if (legacyClubIds.length === 0) {
    return { success: true, migrated: 0, message: 'No legacy clubs to migrate' };
  }

  // Map: old clubId → new clubId (use old ID as new ID if unique, else generate)
  const idMap: Record<string, string> = {};
  const now = new Date().toISOString();
  const updates: Record<string, unknown> = {};
  let firstNewClubId: string | null = null;

  for (const [oldId, legacyClub] of Object.entries(legacyClubs)) {
    // Zkontroluj zda už existuje v /clubs (idempotence)
    const existsSnap = await db.ref(`clubs/${oldId}`).get();
    const newId = existsSnap.exists() ? (db.ref('clubs').push().key!) : oldId;
    idMap[oldId] = newId;
    if (!firstNewClubId) firstNewClubId = newId;

    const sharedClub: SharedClub = {
      id: newId,
      name: legacyClub.name,
      color: legacyClub.color || '#2E7D32',
      logoBase64: legacyClub.logoBase64 || null,
      ownership: 'personal',
      createdAt: (legacyClub as unknown as { createdAt?: string }).createdAt || now,
      createdBy: targetUid,
      updatedAt: now,
      members: {
        [targetUid]: { role: 'owner', joinedAt: now },
      },
      players: legacyClub.players || [],
      ageCategories: legacyClub.ageCategories || [],
      defaultPlayers: legacyClub.defaultPlayers || [],
    };

    updates[`clubs/${newId}`] = sharedClub;
    updates[`users/${targetUid}/memberOfClubs/${newId}`] = 'owner';
  }

  // Migrate trainings: použij training.clubId pokud existuje, jinak first club
  const legacyTrainings = (legacyTrainingsSnap.val() || {}) as Record<string, { clubId?: string }>;
  for (const [trainingId, training] of Object.entries(legacyTrainings)) {
    const targetClubId = (training.clubId && idMap[training.clubId]) || firstNewClubId;
    if (targetClubId) {
      updates[`trainings/${targetClubId}/${trainingId}`] = training;
    }
  }

  // Migrate matches
  const legacyMatches = (legacyMatchesSnap.val() || {}) as Record<string, { clubId?: string }>;
  for (const [matchId, match] of Object.entries(legacyMatches)) {
    const targetClubId = (match.clubId && idMap[match.clubId]) || firstNewClubId;
    if (targetClubId) {
      updates[`matches/${targetClubId}/${matchId}`] = match;
    }
  }

  // Migrate contacts
  const legacyContacts = (legacyContactsSnap.val() || {}) as Record<string, { clubId?: string | null }>;
  for (const [contactId, contact] of Object.entries(legacyContacts)) {
    const targetClubId = (contact.clubId && idMap[contact.clubId]) || firstNewClubId;
    if (targetClubId) {
      updates[`contacts/${targetClubId}/${contactId}`] = contact;
    }
  }

  // Nastav activeClubId na první migrated club
  const activeSnap = await db.ref(`users/${targetUid}/activeClubId`).get();
  if (!activeSnap.exists() && firstNewClubId) {
    updates[`users/${targetUid}/activeClubId`] = firstNewClubId;
  }
  ...
  if (dryRun) {
    return {
      success: true,
      dryRun: true,
      wouldMigrate: legacyClubIds.length,
      idMap,
  ...
  await db.ref().update(updates);
  await audit(callerUid, 'migrateUserClubs', targetUid, {
    clubsCount: legacyClubIds.length,
    idMap,
  });

  return {
    success: true,
    migrated: legacyClubIds.length,
    idMap,
    firstNewClubId,
  };
});
