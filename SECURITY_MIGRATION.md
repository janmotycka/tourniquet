...
**Code má TODO komentáře** kde přesně přidat `.runWith({ secrets: [...] })` — `stripe.ts`, `webhook.ts`, `notifications.ts`. Musíš nejdřív nastavit secrets, **pak** odkomentovat runWith řádky, pak deploy. (Pokud bys přidal runWith dřív, deploy by selhal s "secret not found".)
